...
/* ================= ORIGINAL MODULES (Working Perfectly) ================= */
...
app.post("/add-review", async (req, res) => {
  ...
  await pool.query("INSERT INTO reviews (review_title, emp_id, review_date, review_period, rating, comment) VALUES ($1, $2, $3, $4, $5, $6)", [review_title, emp_id, review_date, review_period, rating, comment]);
  res.send("Review Added");
});

app.get("/reviews", async (req, res) => {
  const result = await pool.query("SELECT r.*, e.emp_name FROM reviews r JOIN employees e ON r.emp_id = e.emp_id WHERE r.is_deleted = FALSE");
  res.json(result.rows);
});
...
/* ================= LEAVE MANAGEMENT (Email Sync Logic) ================= */
...
app.post("/send-otp", async (req, res) => {
  ...
  await transporter.sendMail({ from: process.env.EMAIL_USER, to: email, subject: "OTP", text: `OTP: ${otp}` });
  res.send("OTP sent");
});
